# usq_trigger_2.py
# Example for collecting 6 channels at 2kHz on each of two boards, nodes '8' and '9'.
# PJ 2024-11-13
#
...
def main(sp, node_ids):
    ...
    #
    nodes[0].disable_external_trigger() # depends on node '9' for event signal
    nodes[1].enable_external_trigger(64, 'pos') # on node '9'
    for daq_mcu in daq_mcus:
    ...
    while not ready:
        print('Waiting for DAQ_MCU on node 9...')
        time.sleep(0.1)
        ready = nodes[1].test_DAQ_MCU_is_ready()
    ...
    while not ready:
        ...
        time.sleep(0.1)
        ready = nodes[8].test_DAQ_MCU_is_ready()
    #
    for daq_mcu in daq_mcus:
