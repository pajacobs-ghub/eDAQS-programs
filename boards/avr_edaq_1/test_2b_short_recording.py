...
def main(sp, node_id):
    ...
    bytes_per_sample = daq_mcu.get_byte_size_of_sample_set()
    max_samples = daq_mcu.get_max_nsamples()
    size_of_SRAM = daq_mcu.get_size_of_SRAM_in_bytes()
    print(f"bytes_per_sample={bytes_per_sample}, size_of_SRAM={size_of_SRAM}")
    print(f"max_samples={max_samples}")
    for i in range(nsamples):
        items = daq_mcu.get_formatted_sample(i)
        print(items)
    return
