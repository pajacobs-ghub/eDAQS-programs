# test_8b_4chan_record.py
# Example for collecting four channels of data for the ramjet.
# PJ 2024-10-08, 2025-04-05 adapted test_8 to 4 channels
#
...
def main(sp, node_id):
    ...
    #
    print("Example of recording four channels at 12.5 kHz.")
    ...
    daq_mcu.set_sample_period_us(80)
    daq_mcu.set_analog_ref_voltage('4v096')
    daq_mcu.set_nsamples(16384)
    daq_mcu.set_trigger_internal(0, 1000, 1)
    daq_mcu.set_analog_channels([('AIN28','GND'),('ain29','gnd'),('AIN30','gnd'),('AIN31','GND')])
    daq_mcu.clear_PGA()
    daq_mcu.set_burst(0)
    ...
    ax0.plot(my_samples['data'][0][0:N]); ax0.set_ylabel('chan 0'); ax0.tick_params(labelbottom=False)
    ax1.plot(my_samples['data'][1][0:N]); ax1.set_ylabel('chan 1'); ax1.tick_params(labelbottom=False)
    ax2.plot(my_samples['data'][2][0:N]); ax2.set_ylabel('chan 2'); ax2.tick_params(labelbottom=False)
    ax3.plot(my_samples['data'][3][0:N]); ax3.set_ylabel('chan 3')
...
if __name__ == '__main__':
    ...
    # $ python3 test_8b_4chan_record.py -i 1
    import argparse
